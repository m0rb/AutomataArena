...
async def handle_info_view(node, nickname: str, args: list, reply_target: str):
    ...
    if target == "grid":
        ...
        if machine: await node.send(f"PRIVMSG {tactical_target} :[INFO] ARENA_STATUS:{b_stat} QUEUE:{q_len} READY:{r_len}")
            ...
            await node.send(f"PRIVMSG {tactical_target} :{tag_msg(format_text(f'Status: {b_stat} | Players in Queue: {q_len} | Drop Pods Ready: {r_len}', C_YELLOW), tags=['ARENA'], is_machine=machine)}")
    else:
        ...
        if f.get('race') == "Spectator":
            await handle_spectator_stats(node, nickname, [target], tactical_target)
            return
...
async def handle_options(node, nickname: str, args: list, reply_target: str):
    ...
    if not args:
        if machine:
            ...
            await node.send(f"{tactical_cmd} {tactical_target} :[OPTIONS] {parts}")
        else:
            await node.send(f"PRIVMSG {tactical_target} :{tag_msg(format_text('=== [ACCOUNT OPTIONS] ===', C_CYAN, bold=True), tags=['SIGACT'], is_machine=machine)}")
            labels = {
            ...
            for k, l in labels.items():
                v = prefs.get(k)
                await node.send(f"PRIVMSG {tactical_target} :{tag_msg(f'{l}: {format_text(str(v), C_GREEN if v else C_RED, is_machine=machine)} ', tags=['SIGACT'], is_machine=machine)}")
            await node.send(f"PRIVMSG {tactical_target} :{tag_msg(format_text(f'Use {node.prefix} options <setting> <value> to change.', C_YELLOW), tags=['SIGACT'], is_machine=machine)}")
        return
    if len(args) == 1:
        s = args[0].lower()
        if s in VALID:
            ...
            await node.send(f"{tactical_cmd} {tactical_target} :[OPTIONS] {s} is currently: {current}")
            await node.send(f"{tactical_cmd} {tactical_target} :[SYNTAX] {node.prefix} options {s} <{allowed}>")
        else:
            await node.send(f"{tactical_cmd} {tactical_target} :[ERR] Unknown setting '{s}'.")
        return
    if len(args) < 2:
        await node.send(f"{tactical_cmd} {tactical_target} :[ERR] Syntax: {node.prefix} options <setting> <value>")
        return
    ...
    key, val_map = VALID[s]
    if v not in val_map:
        await node.send(f"{tactical_cmd} {tactical_target} :[ERR] Invalid value '{v}'. Valid: {'/'.join(val_map.keys())}")
        return
    ...
    if machine:
        ...
        await node.send(f"PRIVMSG {reply_target} :{tag_msg(format_text(f'Option {s} updated to {v}.', C_GREEN), tags=['SIGACT', nickname])}")
    if s == "msgtype":
    ...
    if machine or v == "machine": 
        ...
        await node.send(f"PRIVMSG {tactical_target} :{tag_msg(format_text(confirm, C_GREEN), tags=['SIGACT'])}")
...
async def handle_stats(node, nickname: str, args: list, reply_target: str):
    ...
    if args[0].lower() == "allocate":
        ...
        if success:
            await node.send(f"PRIVMSG {tactical_target} :{tag_msg(format_text(f'✔️ Point allocated to {stat.upper()}. Upgrade successful.', C_GREEN), tags=['SIGACT', nickname])}")
            if machine:
                # Public narrative
            ...
            await node.send(f"PRIVMSG {tactical_target} :[ERR] Allocation failed. Verify you have pending points and a valid stat name.")
